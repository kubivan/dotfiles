...
then
  ...
  tmux split-window -t $SESSION_NAME 'sudo tail -f /var/log/Xorg.0.log'
  tmux new-window -c /data/downloads -n media  -t $SESSION_NAME
  tmux new-window -c ~               -n update -t $SESSION_NAME
  tmux new-window                    -n ncmpc  -t $SESSION_NAME ncmpc
  tmux new-window                    -n htop   -t $SESSION_NAME htop
  tmux set-window-option -t $SESSION_NAME:media monitor-activity off
  ...
  tmux set-window-option -t $SESSION_NAME:ncmpc   monitor-activity off
